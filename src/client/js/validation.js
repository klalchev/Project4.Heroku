function validate(myURL) {
    ...
    if (pattern.test(myURL))
     {
        postData('/addSentimentAnalysis', {url:myURL})  //I am using a DevServer in webpack dev environemnt which redirects the
        .then(res=>{updateUI(res)});                    //api request to express server port 4040, hence
    }                                                   //why '/addSentimentAnalysis in build-dev environment
    else                                                //If I dont want to use DevServer in webpack.dev.js,
    {                                                   //then I need to change the relative URL '/addSentimentAnalysis' to a full URL 'http://localhost:4040/addSentimentAnalysis' and it would work without the proxy server
